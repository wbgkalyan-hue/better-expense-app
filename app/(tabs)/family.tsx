...
import { getFamilyMembers, addFamilyMember } from "@/services/firestore"
...
export default function FamilyScreen() {
  ...
  const [formRelationship, setFormRelationship] = useState<FamilyRelationship | "">("")
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setShowAdd(false)
      setFormName(""); setFormRelationship(""); setFormPhone(""); setFormEmail("")
      await load()
    } catch (err: any) {
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: cardBg }]}>
          <Text style={[styles.summaryLabel, { color: colors.icon }]}>Total Members</Text>
      ...
      {items.length === 0 ? (
        <View style={[styles.emptyState, { backgroundColor: cardBg }]}>
          <Text style={[styles.emptyText, { color: colors.icon }]}>No family members added yet.</Text>
        </View>
      ) : (
        items.map((item) => (
          <TouchableOpacity
            ...
            <View style={styles.cardHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.cardSub, { color: colors.icon }]}>
                  {[item.relationship ? FAMILY_RELATIONSHIP_LABELS[item.relationship] : null, item.phone, item.email].filter(Boolean).join(" • ") || "No info"}
                </Text>
              </View>
        ...
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={[styles.modalContent, { backgroundColor: colors.background }]}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.typeScroll}>
              {Object.entries(FAMILY_RELATIONSHIP_LABELS).map(([val, label]) => (
                <TouchableOpacity key={val} style={[styles.typeChip, { backgroundColor: formRelationship === val ? colors.tint : cardBg, borderColor: formRelationship === val ? "transparent" : colors.icon + "40" }]} onPress={() => setFormRelationship(val as FamilyRelationship)}>
