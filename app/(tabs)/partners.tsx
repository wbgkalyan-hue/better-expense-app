...
import { getPartners, addPartner } from "@/services/firestore"
...
export default function PartnersScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.contentContainer}>
      <Text style={[styles.title, { color: colors.text }]}>Partners</Text>

      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: cardBg }]}>
          <Text style={[styles.summaryLabel, { color: colors.icon }]}>Total Partners</Text>
      ...
      {items.length === 0 ? (
        <View style={[styles.emptyState, { backgroundColor: cardBg }]}>
          <Text style={[styles.emptyText, { color: colors.icon }]}>No partners added yet.</Text>
        </View>
      ) : (
        items.map((item) => (
          <TouchableOpacity
            ...
            <View style={styles.cardHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.cardSub, { color: colors.icon }]}>
                  {[item.company, item.phone, item.email].filter(Boolean).join(" • ") || "No info"}
                </Text>
              </View>
      ...
      <Modal visible={showAdd} animationType="slide" transparent>
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={[styles.modalContent, { backgroundColor: colors.background }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>Add Partner</Text>
            ...
            <TextInput style={[styles.input, { color: colors.text, borderColor: colors.icon, backgroundColor: cardBg }]} placeholder="Company (optional)" placeholderTextColor={colors.icon} value={formCompany} onChangeText={setFormCompany} />
